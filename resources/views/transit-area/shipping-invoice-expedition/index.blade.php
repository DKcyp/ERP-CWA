...
@section('title','Shipping Invoice Expedition')
@section('content')
<div class="page-content">
    <div class="card border-0 shadow-sm hz-card mb-4"><div class="card-body">
        <div class="d-flex gap-2 justify-content-end">
            <button type="button" class="btn btn-primary" id="btn-add"><i class="bi bi-plus-lg me-1"></i>Tambah Shipping</button>
        </div>
    </div></div>
...
<div class="modal fade" id="modal-data" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-dialog-scrollable"><div class="modal-content">
        <div class="modal-header"><h5 class="modal-title">Tambah Shipping</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
...
@push('after-script')
<script>
const tableUrl="{{route('shipping-invoice-expedition.table')}}",storeUrl="{{route('shipping-invoice-expedition.store')}}",showUrl="{{route('shipping-invoice-expedition.show','__ID__')}}",updateUrl="{{route('shipping-invoice-expedition.update','__ID__')}}",deleteUrl="{{route('shipping-invoice-expedition.destroy','__ID__')}}",csrf=$('meta[name="csrf-token"]').attr('content');
...
function resetForm(){form[0].reset();idI.val('');form.find('.is-invalid').removeClass('is-invalid');form.find('.invalid-feedback').remove();modal.find('.modal-title').text('Tambah Shipping');}
...
$('#table-data').on('click','.btn-edit',function(){const id=$(this).data('id');resetForm();$.get(showUrl.replace('__ID__',id)).done(function(r){const d=r.data||{};idI.val(d.id);$('#f_doc_id').val(d.doc_id??'');$('#f_date').val(d.date??'');$('#f_wh').val(d.warehouse??'');$('#f_sales').val(d.salesman??'');$('#f_notes').val(d.notes??'');$('#f_user').val(d.user_id??'');modal.find('.modal-title').text('Edit Shipping');modal.modal('show')}).fail(function(){Swal.fire({icon:'error',title:'Gagal',text:'Tidak dapat mengambil data.'})})});
$('#table-data').on('click','.btn-delete',function(){const id=$(this).data('id');Swal.fire({title:'Hapus shipping ini?',text:'Data yang dihapus tidak dapat dikembalikan.',icon:'warning',showCancelButton:true,confirmButtonColor:'#d33',confirmButtonText:'Ya, hapus',cancelButtonText:'Batal'}).then(function(r){if(!r.isConfirmed)return;$.ajax({url:deleteUrl.replace('__ID__',id),method:'POST',data:{_method:'DELETE'},success:function(r2){Swal.fire({icon:'success',title:r2.message||'Data dihapus',timer:1500,showConfirmButton:false});tbl.ajax.reload(null,false)},error:function(x){const r3=x.responseJSON||{};Swal.fire({icon:'error',title:'Gagal',text:r3.message||'Tidak dapat menghapus data.'})}})})});
</script>
@endpush