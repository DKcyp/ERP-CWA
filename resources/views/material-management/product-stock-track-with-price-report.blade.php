...
@section('title','Product Stock Track with Price Report')
@section('content')
<div class="page-content">
    ...
    <div class="card border-0 shadow-sm hz-card"><div class="card-body">
        <div class="table-responsive"><table class="table table-hover align-middle w-100 table-sm" id="table-data"><thead class="table-light">
            <tr><th class="text-center" style="width:30px">No</th><th class="text-center">Trans Date</th><th>Product ID</th><th>Name</th><th>Ref Doc No</th><th>Tipe Transaksi</th><th class="text-end">In Qty (+)</th><th class="text-end">Out Qty (-)</th><th class="text-end">Balance Qty</th><th class="text-end">Unit Cost (IDR)</th><th class="text-end">Total Valuation (IDR)</th></tr>
        </thead></table></div>
    </div></div>
</div>
@endsection
...
$('#table-data').DataTable({processing:true,serverSide:true,scrollX:true,ajax:{url:"{{route('product-stock-track-with-price-report.table')}}",data:function(d){d.filter_search=$('#filter-search').val();d.filter_date_from=$('#filter-date-from').val();d.filter_date_to=$('#filter-date-to').val();d.filter_type=$('#filter-type').val()}},columns:[
...
{data:'total_valuation_fmt',name:'total_valuation',orderable:false,searchable:false,className:'text-end'}
]});
...
function exportCSV(){$.ajax({url:"{{route('product-stock-track-with-price-report.table')}}",data:{filter_search:$('#filter-search').val(),filter_date_from:$('#filter-date-from').val(),filter_date_to:$('#filter-date-to').val(),filter_type:$('#filter-type').val(),start:0,length:10000,draw:1},success:function(r){let csv='No,Trans Date,Product ID,Name,Ref Doc No,Type,In Qty,Out Qty,Balance,Unit Cost IDR,Total Valuation IDR\n';r.data.forEach(function(row){csv+=`"${row.DT_RowIndex}","${row.date_fmt}","${row.product_id}","${row.name}","${row.ref_doc_no}","${(row.type_badge||'').replace(/<[^>]*>/g,'')}","${(row.in_fmt||'').replace(/[^0-9]/g,'')}","${(row.out_fmt||'').replace(/[^0-9]/g,'')}","${row.balance_fmt}","${row.unit_cost_fmt.replace(/[^0-9]/g,'')}","${row.total_valuation_fmt.replace(/[^0-9]/g,'')}"\n`});const blob=new Blob([csv],{type:'text/csv'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='product-stock-track-with-price-report.csv';a.click()}})}
</script>
@endpush