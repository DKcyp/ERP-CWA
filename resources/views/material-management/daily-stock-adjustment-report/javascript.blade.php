...
@push('after-script')
<script>
    const tableUrl = "{{ route('daily-stock-adjustment.table') }}";
    const summaryUrl = "{{ route('daily-stock-adjustment.summary') }}";
    const showUrl  = "{{ route('daily-stock-adjustment.show', '__ID__') }}";
    ...
    const tableDaily = $('#table-daily').DataTable({
        ...
        columns: [
            ...
            { data: 'total_qty_diff',   name: 'total_qty_diff',  className: 'text-center' },
