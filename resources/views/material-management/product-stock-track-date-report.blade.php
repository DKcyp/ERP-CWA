...
@section('title','Product Stock Track Date Report')
@section('content')
<div class="page-content">
    <div class="card border-0 shadow-sm hz-card mb-3"><div class="card-body py-2">
        <div class="row g-2 align-items-end">
            <div class="col-md-2"><label class="form-label fw-semibold mb-1 small text-muted"><i class="bi bi-search me-1"></i>Cari</label><input type="text" class="form-control" id="filter-search" placeholder="Product ID, Nama, Ref..."></div>
            ...
            <div class="col-md-2 d-flex gap-2 justify-content-md-end"><button type="button" class="btn btn-outline-secondary btn-sm" id="btn-reset-filter"><i class="bi bi-arrow-counterclockwise me-1"></i>Reset</button><button class="btn btn-outline-success btn-sm" onclick="exportCSV()"><i class="bi bi-download me-1"></i>Export</button></div>
        </div>
    </div></div>
    <div class="card border-0 shadow-sm hz-card"><div class="card-body">
        <div class="table-responsive"><table class="table table-hover align-middle w-100 table-sm" id="table-data"><thead class="table-light">
            <tr><th class="text-center" style="width:30px">No</th><th class="text-center">Period Date</th><th>Product ID</th><th>Name</th><th>Ref Doc No</th><th>Tipe Transaksi</th><th class="text-end">In Qty (+)</th><th class="text-end">Out Qty (-)</th><th class="text-end">Balance Qty</th><th>Warehouse</th></tr>
        </thead></table></div>
    </div></div>
</div>
@endsection
...
$('#table-data').DataTable({processing:true,serverSide:true,scrollX:true,ajax:{url:"{{route('product-stock-track-date-report.table')}}",data:function(d){d.filter_search=$('#filter-search').val();d.filter_date_from=$('#filter-date-from').val();d.filter_date_to=$('#filter-date-to').val();d.filter_warehouse=$('#filter-warehouse').val();d.filter_type=$('#filter-type').val()}},columns:[
...
$('#btn-reset-filter').on('click',function(){$('#filter-search').val('');$('#filter-date-from').val('');$('#filter-date-to').val('');$('#filter-warehouse').val('all');$('#filter-type').val('all');$('#table-data').DataTable().ajax.reload()});
function exportCSV(){$.ajax({url:"{{route('product-stock-track-date-report.table')}}",data:{filter_search:$('#filter-search').val(),filter_date_from:$('#filter-date-from').val(),filter_date_to:$('#filter-date-to').val(),filter_warehouse:$('#filter-warehouse').val(),filter_type:$('#filter-type').val(),start:0,length:10000,draw:1},success:function(r){let csv='No,Period Date,Product ID,Name,Ref Doc No,Type,In Qty,Out Qty,Balance,Warehouse\n';r.data.forEach(function(row){csv+=`"${row.DT_RowIndex}","${row.date_fmt}","${row.product_id}","${row.name}","${row.ref_doc_no}","${(row.type_badge||'').replace(/<[^>]*>/g,'')}","${(row.in_fmt||'').replace(/[^0-9]/g,'')}","${(row.out_fmt||'').replace(/[^0-9]/g,'')}","${row.balance_fmt}","${row.warehouse}"\n`});const blob=new Blob([csv],{type:'text/csv'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='product-stock-track-date-report.csv';a.click()}})}
</script>
@endpush