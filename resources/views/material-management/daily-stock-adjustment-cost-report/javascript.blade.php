...
@push('after-script')
<script>
    const tableUrl = "{{ route('daily-stock-adjustment-cost.table') }}";
    const summaryUrl = "{{ route('daily-stock-adjustment-cost.summary') }}";
    const showUrl  = "{{ route('daily-stock-adjustment-cost.show', '__ID__') }}";
    ...
    function loadSummary() {
        $.get(summaryUrl, getFilterData())
            .done(function (res) {
                ...
                $('#summary-total-cost-diff').text(res.total_cost_diff || 'Rp 0');
    ...
    const tableDaily = $('#table-daily').DataTable({
        ...
        columns: [
            ...
            { data: 'line_cost_unit_fmt', name: 'line_cost_unit', className: 'text-end' },
            { data: 'line_cost_diff_fmt', name: 'line_cost_diff', className: 'text-end' },
        ...
        $.get(showUrl.replace('__ID__', id))
            .done(function (res) {
                ...
                (d.items || []).forEach(function (item, idx) {
                    ...
                    const cost = parseInt(item.cost_per_unit) || 0;
                    ...
                    tbody.append(`
                        <tr>
                            ...
                            <td class="text-end">${fmt(costDiff)}</td>
                        </tr>
                    `);
                });
                $('#detail-total-cost').text('Rp ' + grandTotal.toLocaleString('id-ID'));
