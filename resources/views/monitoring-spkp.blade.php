...
@section('title','Monitoring SPKP (Quality Control Base / Giling)')
    ...
    <div class="card-body py-2">
        <div class="row g-2 align-items-end">
            ...
            <div class="col-md-2"><label class="form-label form-label-sm">Keputusan</label>
                <select class="form-select form-select-sm" id="filterKeputusan"><option value="all">All</option><option>Approve Base</option><option>Reject Base</option><option>Rework ADU</option></select>
            </div>
            <div class="col-md-7 text-end"><button class="btn btn-sm btn-primary" onclick="openForm()"><i class="bi bi-plus-lg me-1"></i>Tambah Uji SPKP</button></div>
        </div>
    </div>
</div>

<div class="row g-3 mb-3">
    <div class="col-md-3"><div class="card border-start border-4 border-success shadow-sm h-100"><div class="card-body py-2"><small class="text-muted">Approve Base</small><h5 class="fw-bold mb-0 text-success" id="statApprove">-</h5></div></div></div>
    <div class="col-md-3"><div class="card border-start border-4 border-danger shadow-sm h-100"><div class="card-body py-2"><small class="text-muted">Reject Base</small><h5 class="fw-bold mb-0 text-danger" id="statReject">-</h5></div></div></div>
    <div class="col-md-3"><div class="card border-start border-4 border-warning shadow-sm h-100"><div class="card-body py-2"><small class="text-muted">Rework ADU</small><h5 class="fw-bold mb-0 text-warning" id="statRework">-</h5></div></div></div>
        ...
        <div class="table-responsive">
            <table class="table table-hover table-sm mb-0" style="font-size:0.8rem;" id="mainTable">
                <thead class="table-dark">
                    <tr>
                        <th width="20">#</th><th>Product</th><th>Batch No</th><th>Type Prod</th><th>Waktu</th><th>User</th>
                        <th style="min-width:150px;">Parameter Fisikokimia</th><th style="min-width:130px;">Visual & Fisik</th><th>Keputusan</th><th width="100">Aksi</th>
                    </tr>
                </thead>
            </table>
        </div>
    ...
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white py-2">
                <h6 class="modal-title"><i class="bi bi-clipboard-check me-1"></i><span id="modalTitle">Tambah Uji SPKP</span></h6>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <form id="mainForm"><input type="hidden" id="formId">
                    <ul class="nav nav-tabs mb-3" role="tablist">
                        <li class="nav-item"><button class="nav-link active" data-bs-toggle="tab" data-bs-target="#tabGeneral" type="button"><i class="bi bi-info-circle me-1"></i>General & Waktu</button></li>
                        <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#tabChemical" type="button"><i class="bi bi-droplet me-1"></i>Parameter Kimia/Fisika</button></li>
                        <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#tabVisual" type="button"><i class="bi bi-eye me-1"></i>Visual & Fisik Kemasan</button></li>
                        <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#tabFooter" type="button"><i class="bi bi-check2-square me-1"></i>Kesimpulan</button></li>
                    </ul>
                    <div class="tab-content">
                        <div class="tab-pane fade show active" id="tabGeneral">
                            <div class="card border-0 shadow-sm"><div class="card-body"><div class="row g-2">
                                ...
                                <div class="col-md-2"><label class="form-label form-label-sm">Type Production</label><select class="form-select form-select-sm" id="type_production"><option>Base</option><option>Base CM</option><option>Base Pasta</option></select></div>
                        ...
                        <div class="tab-pane fade" id="tabChemical">
                            <div class="card border-0 shadow-sm"><div class="card-body"><div class="row g-2">
...
﻿                        </div>
                        <div class="tab-pane fade" id="tabVisual">
                            ...
                            <div class="card border-0 shadow-sm mb-3"><div class="card-header bg-light py-2"><h6 class="mb-0">Pengujian Aset Pendukung (Kuas/Stapler)</h6></div>
                            <div class="card-body"><div class="row g-2">
                                ...
                                <div class="col-md-2"><label class="form-label form-label-sm">Stapler Test (4-5 bar)</label><select class="form-select form-select-sm" id="stapler_test_4_5"><option>Pass</option><option>Fail</option></select></div>
                            </div><div class="row g-2 mt-1">
                        ...
                        <div class="tab-pane fade" id="tabFooter">
                            <div class="card border-0 shadow-sm"><div class="card-body">
                                <div class="row g-2">
                                    <div class="col-md-8"><label class="form-label form-label-sm">Catatan Laboratorium</label><textarea class="form-control form-control-sm" id="note" rows="4" placeholder="Catatan hasil uji..."></textarea></div>
                                    <div class="col-md-4">
                                        ...
                                        <select class="form-select form-select-sm" id="keputusan" required><option value="Approve Base">Approve Base</option><option value="Reject Base">Reject Base</option><option value="Rework ADU">Rework ADU</option></select>
                                    </div>
                                </div>
                            </div></div>
                        </div>
...
<div class="modal fade" id="detailModal" tabindex="-1">
    <div class="modal-dialog modal-xl"><div class="modal-content">
        <div class="modal-header bg-info text-white py-2"><h6 class="modal-title"><i class="bi bi-eye me-1"></i>Detail Monitoring SPKP</h6><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
...
$(function(){
    table=$('#mainTable').DataTable({processing:true,serverSide:true,
        ajax:{url:'{{route("monitoring-spkp.table")}}',data:function(d){d.filter_search=$('#filterSearch').val();d.filter_keputusan=$('#filterKeputusan').val();}},
        columns:[
...
function loadStats(){
    $.get('{{route("monitoring-spkp.table")}}',{draw:1,start:0,length:5000,'columns[0][data]':'DT_RowIndex','order[0][column]':1,'order[0][dir]':'asc'},function(r){
        var d=r.data||[];var a=0,re=0,ro=0;
        d.forEach(function(i){if(i.keputusan==='Approve Base')a++;else if(i.keputusan==='Reject Base')re++;else ro++;});
        $('#statApprove').text(a);$('#statReject').text(re);$('#statRework').text(ro);$('#statTotal').text(d.length);
    });
}
function openForm(){$('#modalTitle').text('Tambah Uji SPKP');$('#mainForm')[0].reset();$('#formId').val('');$('button[data-bs-target="#tabGeneral"]').tab('show');new bootstrap.Modal('#formModal').show();}
function editRecord(id){
    $.get('{{url("/monitoring-spkp")}}/'+id,function(d){
        $('#modalTitle').text('Edit Uji SPKP');$('#formId').val(d.id);
...
function detailRecord(id){
    $.get('{{url("/monitoring-spkp")}}/'+id,function(d){
        var kb={'Approve Base':'bg-success','Reject Base':'bg-danger','Rework ADU':'bg-warning'};
        ...
        var chem=[['Fineness',d.fineness,'u'],['Viskositas',d.viskositas_ku,'ku'],['SG',d.sg,'gr/ml'],['pH',d.ph,''],['Solid',d.solid_content,'%'],['Gloss',d.gloss,'%'],['Hiding Power',d.hiding_power,'%'],['Colour Str.',d.colour_strenght,'%'],['Visk Detik',d.viskositas_detik,''],['Visk NK2',d.viskositas_nk2,''],['Cycle',d.cycle_time,'mnt'],['Colour',d.colour,'']];
        chem.forEach(function(c){h+='<div class="col"><small class="text-muted">'+c[0]+'</small><br><strong>'+(c[1]||'-')+' '+c[2]+'</strong></div>';});
        h+='</div></div></div>';
        h+='<div class="card border-0 shadow-sm mb-3"><div class="card-header bg-warning bg-opacity-10 py-2"><h6 class="mb-0 text-warning">Visual & Fisik Kemasan</h6></div><div class="card-body py-2"><div class="row g-2">';
        var vis=[['Layout',d.layout],['Adhesi',d.adhesi],['Drop',d.drop_test],['Seep',d.seep_test],['Ball',d.ball_test],['Matching',d.matching_test],['Miss Print',d.miss_print],['Teks',d.teks],['Tampilan',d.tampilan],['Kebersihan',d.kebersihan_kemasan],['Kualitas Cetakan',d.kualitas_cetakan],['Stapler',d.stapler_test]];
...
function saveForm(){
    ...
    var url=id?'{{url("/monitoring-spkp")}}/'+id:'{{route("monitoring-spkp.store")}}';
...
function deleteRecord(id){if(!confirm('Hapus data ini?'))return;$.ajax({url:'{{url("/monitoring-spkp")}}/'+id,method:'DELETE',data:{_method:'DELETE'},success:function(r){table.ajax.reload();loadStats();showToast(r.message||'Dihapus','success');}});}
</script>
@endpush