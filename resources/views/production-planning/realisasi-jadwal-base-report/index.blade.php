...
@section('title','Realisasi Jadwal Base Report')
@section('content')
<div class="page-content">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-3"><div class="card border-0 shadow-sm hz-card h-100"><div class="card-body">
            ...
            <div><p class="text-muted mb-0 small">Rata-rata Efisiensi</p><h4 class="fw-bold mb-0" id="sum-eff">0%</h4></div></div>
        </div></div></div>
        <div class="col-md-3"><div class="card border-0 shadow-sm hz-card h-100"><div class="card-body">
            <div class="d-flex align-items-center"><div class="flex-shrink-0 me-3"><div class="avatar bg-warning bg-opacity-10 text-warning rounded-circle"><i class="bi bi-arrow-left-right fs-4"></i></div></div>
            <div><p class="text-muted mb-0 small">Total Variance (KG)</p><h4 class="fw-bold mb-0" id="sum-var">0</h4></div></div>
        </div></div></div>
    </div>
    ...
    <div class="card border-0 shadow-sm hz-card"><div class="card-body">
        <div class="table-responsive"><table class="table table-hover align-middle w-100 table-sm" id="table-data"><thead class="table-light">
            <tr><th class="text-center" style="width:30px">No</th><th>Doc ID</th><th>Prod Date</th><th>Shift</th><th>Type</th><th>Product</th><th>Batch No</th><th>Mesin</th><th class="text-end">Basis (KG)</th><th class="text-end">Realisasi (KG)</th><th class="text-center">Variance</th><th class="text-center">Efficiency</th><th class="text-center">Jam</th><th>Operator</th><th>Notes</th></tr>
        </thead></table></div>
    </div></div>
</div>
@endsection
...
$.get('{{route("realisasi-jadwal-base-report.table")}}',{filter_search:'',filter_date_from:'',filter_date_to:'',filter_shift:'all',filter_type:'all',filter_mesin:'all',filter_operator:'all',draw:1,start:0,length:100},function(init){
...
const tbl=$('#table-data').DataTable({processing:true,serverSide:true,scrollX:true,ajax:{url:"{{route('realisasi-jadwal-base-report.table')}}",data:function(d){d.filter_search=$('#filter-search').val();d.filter_date_from=$('#filter-date-from').val();d.filter_date_to=$('#filter-date-to').val();d.filter_shift=$('#filter-shift').val();d.filter_type=$('#filter-type').val();d.filter_mesin=$('#filter-mesin').val();d.filter_operator=$('#filter-operator').val()},dataSrc:function(json){const s=json.summary||{};$('#sum-basis').text((s.total_basis||0).toLocaleString('id-ID'));$('#sum-kg').text((s.total_realisasi||0).toLocaleString('id-ID'));$('#sum-eff').text((s.avg_eff||0)+'%');const v=s.total_variance||0;$('#sum-var').text((v>0?'+':'')+v.toLocaleString('id-ID')).removeClass('text-danger text-success').addClass(v>0?'text-success':v<0?'text-danger':'');return json.data}},columns:[
...
{data:null,orderable:false,searchable:false,className:'text-center',render:function(d){return d.mulai+' - '+d.selesai}},
...
$('#btn-reset-filter').on('click',function(){$('#filter-search').val('');$('#filter-date-from').val('');$('#filter-date-to').val('');$('#filter-shift').val('all');$('#filter-type').val('all');$('#filter-mesin').val('all');$('#filter-operator').val('all');tbl.ajax.reload()});
</script>
@endpush