...
@section('title','Realisasi Jadwal CM List')
@section('content')
<div class="page-content">
    <div class="card border-0 shadow-sm hz-card mb-4"><div class="card-body">
        <div class="row g-3 align-items-end">
            ...
            <div class="col-md-2"><label class="form-label fw-semibold mb-1 small text-muted">Lokasi Jadwal</label><select class="form-select form-select-sm" id="filter-category"><option value="all">Semua</option><option value="Pusat">Pusat</option><option value="Cabang">Cabang</option></select></div>
            <div class="col-md-2 d-flex gap-2 justify-content-md-end"><button type="button" class="btn btn-outline-secondary btn-sm" id="btn-reset-filter"><i class="bi bi-arrow-counterclockwise me-1"></i>Reset</button><button type="button" class="btn btn-primary" id="btn-add"><i class="bi bi-plus-lg me-1"></i>Tambah Realisasi CM</button></div>
        </div>
    </div></div>
    <div class="card border-0 shadow-sm hz-card"><div class="card-body">
        <div class="table-responsive"><table class="table table-hover align-middle w-100 table-sm" id="table-data"><thead class="table-light">
            <tr><th class="text-center" style="width:30px">No</th><th>Doc ID</th><th>Date</th><th>User ID</th><th>Prod Date</th><th>Shift</th><th>Type</th><th>Jadwal</th><th class="text-center">Total Produk</th><th class="text-end">Total Realisasi (KG)</th><th>Status</th><th>Notes</th><th class="text-center">Aksi</th></tr>
        </thead></table></div>
    </div></div>
</div>
...
<div class="modal fade" id="formModal" tabindex="-1"><div class="modal-dialog modal-xl modal-dialog-scrollable"><div class="modal-content">
    <div class="modal-header"><h5 class="modal-title fw-bold" id="modal-title">Tambah Realisasi CM</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
    <div class="modal-body">
        ...
        <div class="card bg-primary bg-opacity-10 border-0 mb-3"><div class="card-body py-2">
            <div class="row g-3 align-items-end">
                ...
                <div class="col-md-3"><label class="form-label fw-semibold small">User ID</label><input type="text" class="form-control" id="form-user-id" value="cm-001" readonly></div>
        ...
        <div class="card border-0 shadow-sm mb-3"><div class="card-header bg-white py-2 d-flex justify-content-between align-items-center">
            ...
            <div class="table-responsive"><table class="table table-bordered table-sm mb-0" id="detail-table"><thead class="table-light">
                <tr><th class="text-center" style="width:30px">#</th><th>Nama Product <span class="text-danger">*</span></th><th>Kode Warna</th><th>Batch No</th><th>Mesin</th><th class="text-end">Total Basis (KG)</th><th class="text-end">Realisasi (KG) <span class="text-danger">*</span></th><th>Jam Mulai</th><th>Jam Selesai</th><th>Operator</th><th>Jadwal (Ref)</th><th>Keterangan</th><th class="text-center" style="width:35px"></th></tr>
    ...
    <div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal"><i class="bi bi-x-lg me-1"></i>Batal</button><button type="button" class="btn btn-primary" id="btn-save"><i class="bi bi-check-lg me-1"></i>Simpan Realisasi CM</button></div>
</div></div></div>
...
function productSelect(selected){
    ...
    products.forEach((p,i)=>{h+='<option value="'+p+'" data-kode="'+codes[i]+'"'+(p===selected?' selected':'')+'>'+p+'</option>'});
    return h+'</select>';
}
...
function addRow(data){
    ...
    tr.append('<td><input type="text" class="form-control form-control-sm detail-kode" value="'+(data.kode_warna||'')+'" readonly></td>');
    ...
    tr.append('<td><input type="text" class="form-control form-control-sm detail-jadwal" value="'+(data.jadwal_ref||'')+'" readonly></td>');
    ...
    tr.find('.detail-product').on('change',function(){
        const idx=this.selectedIndex-1;
        if(idx>=0)tr.find('.detail-kode').val(codes[idx]);
    });
}
...
function getItems(){
    ...
    $('#detail-body tr').each(function(){
        items.push({
            ...
            kode_warna:$(this).find('.detail-kode').val(),
            ...
            jadwal_ref:$(this).find('.detail-jadwal').val(),
...
$('#btn-add').on('click',function(){
    $('#modal-title').text('Tambah Realisasi CM');
...
$('#btn-load-jadwal').on('click',function(){
    ...
    for(let i=0;i<count;i++){
        ...
        addRow({product_name:products[pi],kode_warna:codes[pi],batch_no:'CM-ADN-'+String(Math.floor(Math.random()*900)+100),machine:machines[Math.floor(Math.random()*machines.length)],total_basis_kg:Math.floor(Math.random()*300)+100,realisasi_kg:Math.floor(Math.random()*300)+100,jam_mulai:String(6+i*4).padStart(2,'0')+':00',jam_selesai:String(9+i*4).padStart(2,'0')+':00',operator:operators[Math.floor(Math.random()*operators.length)],jadwal_ref:$('#form-category').val()});
    }
});

function editRecord(id){
    $.get('/realisasi-jadwal-cm-list/'+id,function(d){
        $('#modal-title').text('Edit Realisasi CM');
        ...
        $('#form-type').val(d.type);$('#form-category').val(d.schedule_category);
        $('#form-status').val(d.status);$('#form-notes').val(d.notes||'');
        $('#detail-body').html('');
...
$('#btn-save').on('click',function(){
    ...
    const payload={
        ...
        type:$('#form-type').val(),schedule_category:$('#form-category').val(),
        status:$('#form-status').val(),notes:$('#form-notes').val(),items:getItems()
    };
    ...
    const url=id?'/realisasi-jadwal-cm-list/'+id:'{{route("realisasi-jadwal-cm-list.store")}}';
...
function deleteRecord(id){deleteId=id;$('#delete-name').text('dokumen CM ini');new bootstrap.Modal('#deleteModal').show();}
$('#btn-confirm-delete').on('click',function(){
    $.ajax({url:'/realisasi-jadwal-cm-list/'+deleteId,type:'POST',data:{_method:'DELETE'},success:function(r){bootstrap.Modal.getInstance('#deleteModal').hide();tbl.ajax.reload(null,false);alert(r.message)}});
});
