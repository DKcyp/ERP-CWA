...
@section('title','Monitoring Mesin Grinding List')
@section('content')
<div class="page-content">
    <div class="card border-0 shadow-sm hz-card mb-4"><div class="card-body">
        <div class="row g-2 align-items-end">
            ...
            <div class="col-md-3 d-flex gap-2 justify-content-md-end"><button type="button" class="btn btn-outline-secondary btn-sm" id="btn-reset-filter"><i class="bi bi-arrow-counterclockwise me-1"></i>Reset</button><button type="button" class="btn btn-primary" id="btn-add"><i class="bi bi-plus-lg me-1"></i>Tambah Monitoring</button></div>
        </div>
    </div></div>
    <div class="card border-0 shadow-sm hz-card"><div class="card-body">
        <div class="table-responsive"><table class="table table-hover align-middle w-100 table-sm" id="table-data"><thead class="table-light">
            <tr><th class="text-center" style="width:30px">No</th><th>Doc ID</th><th>Date</th><th>User ID</th><th>Prod Date</th><th>Shift</th><th>Mesin</th><th>Type</th><th class="text-center">Total Produk</th><th class="text-end">Total Tonase</th><th>Status</th><th>Notes</th><th class="text-center">Aksi</th></tr>
        </thead></table></div>
    </div></div>
</div>
...
<div class="modal fade" id="formModal" tabindex="-1"><div class="modal-dialog modal-fullscreen-lg-down modal-xl modal-dialog-scrollable"><div class="modal-content">
    <div class="modal-header"><h5 class="modal-title fw-bold" id="modal-title">Tambah Monitoring Grinding</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
    <div class="modal-body">
        ...
        <div class="card bg-primary bg-opacity-10 border-0 mb-3"><div class="card-body py-2">
            <div class="row g-3 align-items-end">
                ...
                <div class="col-md-3"><label class="form-label fw-semibold small">User ID</label><input type="text" class="form-control" id="form-user-id" value="gr-001" readonly></div>
        ...
        <div class="card border-0 shadow-sm mb-3"><div class="card-body py-2">
            <div class="row g-2 align-items-end">
                ...
                <div class="col-md-2"><label class="form-label fw-semibold small">Mesin <span class="text-danger">*</span></label><select class="form-select" id="form-machine"><option value="GR-01">GR-01</option><option value="GR-02">GR-02</option><option value="GR-03">GR-03</option></select></div>
        ...
        <div class="card border-0 shadow-sm mb-3"><div class="card-header bg-white py-2 d-flex justify-content-between align-items-center">
            ...
            <div class="table-responsive" style="max-height:400px;overflow-y:auto;"><table class="table table-bordered table-sm mb-0" id="detail-table"><thead class="table-light" style="position:sticky;top:0;z-index:1;">
                <tr>
                    ...
                    <th>Date</th><th>Nama Product <span class="text-danger">*</span></th><th>Batch No</th><th class="text-end">Tonase (Ton)</th><th>No. Mesin</th>
                    <th>Mulai</th><th>Finish</th><th class="text-center">Ke-</th><th class="text-center">Jam</th>
                    <th class="text-center">Gear Pump (RPM)</th><th class="text-center">Blade (RPM)</th>
                    <th class="text-center">Hasil (u)</th>
                    <th>Operator</th><th>Notes</th>
...
const products=['Wall Paint White 20L','Wall Paint Cream 10L','Primer Grey 5L','Top Coat Clear 15L','Cat Ekonomis 5L','Pigment Red Oxide','Pigment Yellow Ochre'];
const operators=['Budi Santoso','Andi Kurniawan','Citra Dewi','Dedi Kuswanto','Eka Putri','Fajar Nugroho'];
let deleteId=null;

const tbl=$('#table-data').DataTable({processing:true,serverSide:true,scrollX:true,ajax:{url:"{{route('monitoring-mesin-grinding-list.table')}}",data:function(d){d.filter_search=$('#filter-search').val();d.filter_date_from=$('#filter-date-from').val();d.filter_date_to=$('#filter-date-to').val();d.filter_shift=$('#filter-shift').val();d.filter_machine=$('#filter-machine').val();d.filter_type=$('#filter-type').val()}},columns:[
...
{data:'tonase_fmt',name:'total_tonase',orderable:false,searchable:false,className:'text-end'},
...
function addRow(data){
    ...
    tr.append('<td><input type="number" class="form-control form-control-sm d-tonase text-end" value="'+(data.tonase||0)+'" step="0.01"></td>');
    tr.append('<td><input type="text" class="form-control form-control-sm d-mesin" value="'+(data.no_mesin||$('#form-machine').val()||'')+'" readonly></td>');
    tr.append('<td><input type="time" class="form-control form-control-sm d-mulai" value="'+(data.mulai||'')+'"></td>');
    tr.append('<td><input type="time" class="form-control form-control-sm d-finish" value="'+(data.finish||'')+'"></td>');
    tr.append('<td><input type="number" class="form-control form-control-sm d-ke text-center" value="'+(data.ke||(i))+'"></td>');
    tr.append('<td><input type="number" class="form-control form-control-sm d-jam text-center" value="'+(data.jam||0)+'" step="0.1"></td>');
    tr.append('<td><input type="number" class="form-control form-control-sm d-gear text-center" value="'+(data.speed_gear_pump||1200)+'"></td>');
    tr.append('<td><input type="number" class="form-control form-control-sm d-blade text-center" value="'+(data.speed_blade||1800)+'"></td>');
    tr.append('<td><input type="number" class="form-control form-control-sm d-micron text-center" value="'+(data.hasil_micron||0)+'" step="0.1"></td>');
    tr.append('<td>'+opSelect(data.operator)+'</td>');
    tr.append('<td><input type="text" class="form-control form-control-sm d-notes" value="'+(data.notes||'')+'"></td>');
...
function getItems(){
    ...
    $('#detail-body tr').each(function(){
        items.push({
            date:$(this).find('.d-date').val(),product_name:$(this).find('.d-product').val(),
            batch_no:$(this).find('.d-batch').val(),tonase:parseFloat($(this).find('.d-tonase').val())||0,
            no_mesin:$(this).find('.d-mesin').val(),mulai:$(this).find('.d-mulai').val(),
            finish:$(this).find('.d-finish').val(),ke:parseInt($(this).find('.d-ke').val())||0,
            jam:parseFloat($(this).find('.d-jam').val())||0,
            speed_gear_pump:parseInt($(this).find('.d-gear').val())||0,
            speed_blade:parseInt($(this).find('.d-blade').val())||0,
            hasil_micron:parseFloat($(this).find('.d-micron').val())||0,
            operator:$(this).find('.d-operator').val(),notes:$(this).find('.d-notes').val(),
        });
    });
    return items;
}
...
$('#btn-add').on('click',function(){
    $('#modal-title').text('Tambah Monitoring Grinding');
...
$('#btn-load-jadwal').on('click',function(){
    ...
    for(let i=0;i<Math.floor(Math.random()*3)+2;i++){
        addRow({product_name:products[Math.floor(Math.random()*products.length)],batch_no:'GR-ADN-'+String(Math.floor(Math.random()*900)+100),tonase:Math.round((Math.random()*1500+500)/100*100)/100,no_mesin:$('#form-machine').val(),mulai:String(6+i*4).padStart(2,'0')+':00',finish:String(6+i*4+3).padStart(2,'0')+':30',ke:i+1,jam:Math.round((Math.random()*35+10)/10*10)/10,speed_gear_pump:Math.floor(Math.random()*700)+800,speed_blade:Math.floor(Math.random()*1500)+1000,hasil_micron:Math.round((Math.random()*45+5)/10*10)/10,operator:operators[Math.floor(Math.random()*operators.length)]});
    }
});

function editRecord(id){$.get('/monitoring-mesin-grinding-list/'+id,function(d){
    $('#modal-title').text('Edit Monitoring Grinding');
...
$('#btn-save').on('click',function(){
    ...
    const url=id?'/monitoring-mesin-grinding-list/'+id:'{{route("monitoring-mesin-grinding-list.store")}}';
...
$('#btn-confirm-delete').on('click',function(){$.ajax({url:'/monitoring-mesin-grinding-list/'+deleteId,type:'POST',data:{_method:'DELETE'},success:function(r){bootstrap.Modal.getInstance('#deleteModal').hide();tbl.ajax.reload(null,false);alert(r.message)}});});
