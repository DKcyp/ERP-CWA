    ...
    <div class="card border-0 shadow-sm hz-card mb-4"><div class="card-body">
        <div class="row g-3 align-items-end">
            <div class="col-md-3">
                ...
                <input type="text" class="form-control" id="filter-search" placeholder="Cari Doc ID atau Warehouse...">
            </div>
            <div class="col-md-2">
            ...
            <div class="col-md-3 d-flex gap-2 justify-content-md-end">
                ...
                <button type="button" class="btn btn-primary" id="btn-add"><i class="bi bi-plus-lg me-1"></i>Tambah SPK</button>
            </div>
        </div>
    </div></div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle w-100 table-sm" id="table-data">
                <thead class="table-light">
                    <tr><th class="text-center">No</th><th>Doc. ID</th><th>Date</th><th>Delivery Date</th><th>Warehouse</th><th class="text-center">Total Qty</th><th class="text-center">Total Tonase</th><th class="text-center">Qty Needs</th><th class="text-center">Tonase Needs</th><th>User ID</th><th class="text-center">Status</th><th class="text-end">Aksi</th></tr>
                </thead>
            </table>
        </div>
    </div></div>
...
<div class="modal fade" id="modal-data" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-xl modal-dialog-scrollable"><div class="modal-content">
        <div class="modal-header"><h5 class="modal-title">Tambah SPK</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <form id="form-data" action="javascript:onSave()">
            <div class="modal-body">@csrf<input type="hidden" id="data_id">
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-3"><label class="form-label fw-semibold">Delivery Date <span class="text-danger">*</span></label><input type="date" class="form-control" id="f_delivery_date"></div>
                    <div class="col-md-3"><label class="form-label fw-semibold">Warehouse <span class="text-danger">*</span></label>
                        <select class="form-select" id="f_warehouse"><option value="Gudang Utama">Gudang Utama</option><option value="Gudang Bahan Jadi">Gudang Bahan Jadi</option><option value="Gudang Cabang">Gudang Cabang</option></select>
                    </div>
                    ...
                    <div class="col-md-3"><label class="form-label fw-semibold">Status <span class="text-danger">*</span></label>
                        <select class="form-select" id="f_status"><option value="DRAFT">Draft</option><option value="PLANNED">Planned</option><option value="IN_PROGRESS">In Progress</option><option value="QC_PENDING">QC Pending</option><option value="COMPLETED">Completed</option><option value="REJECTED">Rejected</option></select>
                    </div>
                ...
                <div class="card shadow-sm border-0 mb-3"><div class="card-body p-3">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <div><label class="form-label fw-semibold mb-1">Detail Items</label><div class="small text-muted">Isi qty dan tonase aktual, serta kebutuhan material (needs).</div></div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover align-middle mb-0" id="table-items">
                            <thead class="table-secondary"><tr>
                                ...
                                <th style="width:90px;" class="text-center">Tonase <span class="text-danger">*</span></th>
                                <th style="width:100px;" class="text-center">Qty Needs</th>
                                <th style="width:100px;" class="text-center">Tonase Needs</th>
                                <th style="width:60px;" class="text-center">Aksi</th>
                            </tr></thead>
                            <tbody id="items-tbody"><tr id="row-empty"><td colspan="8" class="text-center py-4 text-muted"><i class="bi bi-inbox fs-3 d-block mb-2"></i><span class="small">Belum ada item.</span></td></tr></tbody>
                        </table>
                    </div>
                </div></div>
            </div>
...
@push('after-script')
<script>
const tableUrl="{{route('spk-list.table')}}",storeUrl="{{route('spk-list.store')}}",showUrl="{{route('spk-list.show','__ID__')}}",updateUrl="{{route('spk-list.update','__ID__')}}",deleteUrl="{{route('spk-list.destroy','__ID__')}}",csrf=$('meta[name="csrf-token"]').attr('content');
...
{data:'doc_id',name:'doc_id'},{data:'date_fmt',name:'date',className:'text-center'},{data:'delivery_date_fmt',name:'delivery_date',className:'text-center'},
{data:'warehouse',name:'warehouse'},
{data:'total_qty',name:'total_qty',className:'text-center'},{data:'total_tonase',name:'total_tonase',className:'text-center'},
{data:'total_qty_needs',name:'total_qty_needs',className:'text-center',render:function(d){return d?('<span class="text-danger fw-bold">'+d+'</span>'):('<span class="text-success">0</span>')}},
{data:'total_tonase_needs',name:'total_tonase_needs',className:'text-center',render:function(d){return d?('<span class="text-danger fw-bold">'+d+'</span>'):('<span class="text-success">0</span>')}},
...
function addItemRow(data){
    ...
    const pid=data?.product_id??'';const pname=data?.product_name??'';const qty=data?.qty??'';const tonase=data?.tonase??'';const qn=data?.qty_needs??0;const tn=data?.tonase_needs??0;
    tbody.append(`<tr><td class="text-center item-no">${tbody.find('tr:visible').length+1}</td>
        ...
        <td><input type="number" class="form-control form-control-sm item-qty-needs" value="${qn}" placeholder="0" min="0"></td>
        <td><input type="number" class="form-control form-control-sm item-tonase-needs" value="${tn}" placeholder="0" min="0" step="0.01"></td>
...
function collectItems(){const items=[];$('#items-tbody tr:visible').not('#row-empty').each(function(){const pid=$(this).find('.item-product-id').val()||'';const pname=$(this).find('.item-product-name').val()||'';const q=parseInt($(this).find('.item-qty').val())||0;const t=parseFloat($(this).find('.item-tonase').val())||0;const qn=parseInt($(this).find('.item-qty-needs').val())||0;const tn=parseFloat($(this).find('.item-tonase-needs').val())||0;if(pid)items.push({product_id:pid,product_name:pname,qty:q,tonase:t,qty_needs:qn,tonase_needs:tn})});return items}

const modal=$('#modal-data'),form=$('#form-data'),idI=$('#data_id');
function resetForm(){form[0].reset();idI.val('');form.find('.is-invalid').removeClass('is-invalid');form.find('.invalid-feedback').remove();modal.find('.modal-title').text('Tambah SPK');resetItems()}
...
window.onSave=function(){
    ...
    const fd=new FormData();fd.append('_token',csrf);fd.append('doc_id',document.getElementById('f_doc_id').value);fd.append('date',document.getElementById('f_date').value);fd.append('delivery_date',document.getElementById('f_delivery_date').value);fd.append('warehouse',document.getElementById('f_warehouse').value);fd.append('user_id',document.getElementById('f_user_id').value);fd.append('notes',document.getElementById('f_notes').value);fd.append('status',document.getElementById('f_status').value);fd.append('items',JSON.stringify(collectItems()));
...
$('#table-data').on('click','.btn-edit',function(){const id=$(this).data('id');resetForm();$.get(showUrl.replace('__ID__',id)).done(function(r){const d=r.data||{};idI.val(d.id);$('#f_doc_id').val(d.doc_id??'');$('#f_date').val(d.date??'');$('#f_delivery_date').val(d.delivery_date??'');$('#f_warehouse').val(d.warehouse??'Gudang Utama');$('#f_user_id').val(d.user_id??'');$('#f_notes').val(d.notes??'');$('#f_status').val(d.status??'DRAFT');populateItems(d.items??[]);modal.find('.modal-title').text('Edit SPK');modal.modal('show')}).fail(function(){Swal.fire({icon:'error',title:'Gagal',text:'Tidak dapat mengambil data.'})})});

$('#table-data').on('click','.btn-delete',function(){const id=$(this).data('id');Swal.fire({title:'Hapus SPK ini?',text:'Data yang dihapus tidak dapat dikembalikan.',icon:'warning',showCancelButton:true,confirmButtonColor:'#d33',confirmButtonText:'Ya, hapus',cancelButtonText:'Batal'}).then(function(r){if(!r.isConfirmed)return;$.ajax({url:deleteUrl.replace('__ID__',id),method:'POST',data:{_method:'DELETE'},success:function(r2){Swal.fire({icon:'success',title:r2.message||'Data dihapus',timer:1500,showConfirmButton:false});tbl.ajax.reload(null,false)},error:function(x){Swal.fire({icon:'error',title:'Gagal',text:(x.responseJSON||{}).message||'Gagal menghapus.'})}})})});
</script>
@endpush