    ...
    <div class="card border-0 shadow-sm hz-card mb-4"><div class="card-body">
        <div class="row g-3 align-items-end">
            ...
            <div class="col-md-8 d-flex gap-2 justify-content-md-end">
                ...
                <button type="button" class="btn btn-primary" id="btn-add"><i class="bi bi-plus-lg me-1"></i>Tambah Currency</button>
            </div>
        </div>
    </div></div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle w-100 table-sm" id="table-data">
                <thead class="table-light">
                    <tr><th class="text-center">No</th><th>Currency ID</th><th>Code</th><th>Name</th><th>Symbol</th><th>Is Default</th><th>Active</th><th class="text-end">Aksi</th></tr>
                </thead>
            </table>
        </div>
    </div></div>
...
<div class="modal fade" id="modal-data" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-dialog-scrollable"><div class="modal-content">
        <div class="modal-header"><h5 class="modal-title">Tambah Currency</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <form id="form-data" action="javascript:onSave()">
            <div class="modal-body">@csrf<input type="hidden" id="data_id">
                <div class="row g-3">
                    <div class="col-6"><label class="form-label fw-semibold">Code <span class="text-danger">*</span></label><input type="text" class="form-control" name="code" id="f_code" maxlength="10"></div>
                    ...
                    <div class="col-6"><label class="form-label fw-semibold">Symbol <span class="text-danger">*</span></label><input type="text" class="form-control" name="symbol" id="f_symbol" maxlength="10"></div>
                    <div class="col-6"><label class="form-label fw-semibold">Is Default <span class="text-danger">*</span></label>
                        <select class="form-select" name="is_default" id="f_is_default"><option value="N">N - Tidak</option><option value="Y">Y - Ya</option></select>
                    </div>
...
@push('after-script')
<script>
const tableUrl="{{route('currency.table')}}",storeUrl="{{route('currency.store')}}",showUrl="{{route('currency.show','__ID__')}}",updateUrl="{{route('currency.update','__ID__')}}",deleteUrl="{{route('currency.destroy','__ID__')}}",csrf=$('meta[name="csrf-token"]').attr('content');
...
{data:'currency_id',name:'currency_id'},{data:'code',name:'code'},{data:'name',name:'name'},{data:'symbol',name:'symbol'},
{data:'is_default',name:'is_default',render:function(d){return d==='Y'?'<span class="badge bg-primary">Default</span>':'<span class="badge bg-secondary">-</span>'}},
...
function resetForm(){form[0].reset();idI.val('');form.find('.is-invalid').removeClass('is-invalid');form.find('.invalid-feedback').remove();modal.find('.modal-title').text('Tambah Currency');}
...
$('#table-data').on('click','.btn-edit',function(){const id=$(this).data('id');resetForm();$.get(showUrl.replace('__ID__',id)).done(function(r){const d=r.data||{};idI.val(d.id);$('#f_code').val(d.code??'');$('#f_name').val(d.name??'');$('#f_symbol').val(d.symbol??'');$('#f_is_default').val(d.is_default??'N');$('#f_active').val(d.active??'Y');modal.find('.modal-title').text('Edit Currency');modal.modal('show')}).fail(function(){Swal.fire({icon:'error',title:'Gagal',text:'Tidak dapat mengambil data.'})})});
$('#table-data').on('click','.btn-delete',function(){const id=$(this).data('id');Swal.fire({title:'Hapus currency ini?',text:'Data yang dihapus tidak dapat dikembalikan.',icon:'warning',showCancelButton:true,confirmButtonColor:'#d33',confirmButtonText:'Ya, hapus',cancelButtonText:'Batal'}).then(function(r){if(!r.isConfirmed)return;$.ajax({url:deleteUrl.replace('__ID__',id),method:'POST',data:{_method:'DELETE'},success:function(r2){Swal.fire({icon:'success',title:r2.message||'Data dihapus',timer:1500,showConfirmButton:false});tbl.ajax.reload(null,false)},error:function(x){const r3=x.responseJSON||{};Swal.fire({icon:'error',title:'Gagal',text:r3.message||'Tidak dapat menghapus data.'})}})})});
</script>
@endpush