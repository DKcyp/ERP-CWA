    ...
    <div class="card border-0 shadow-sm hz-card mb-4"><div class="card-body">
        <div class="row g-3 align-items-end">
            <div class="col-md-4">
                ...
                <input type="text" class="form-control" id="filter-search" placeholder="Cari brand ID, code, atau nama...">
            </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle w-100 table-sm" id="table-data">
                <thead class="table-light">
                    <tr><th class="text-center">No</th><th>Brand ID</th><th>Code</th><th>Name</th><th>Description</th><th>Active</th><th class="text-end">Aksi</th></tr>
                </thead>
            </table>
        </div>
    </div></div>
...
@push('after-script')
<script>
const tableUrl="{{route('brand.table')}}",storeUrl="{{route('brand.store')}}",showUrl="{{route('brand.show','__ID__')}}",updateUrl="{{route('brand.update','__ID__')}}",deleteUrl="{{route('brand.destroy','__ID__')}}",csrf=$('meta[name="csrf-token"]').attr('content');
...
{data:'brand_id',name:'brand_id'},{data:'code',name:'code'},{data:'name',name:'name'},{data:'description',name:'description',render:function(d){return d||'-'}},{data:'active_badge',name:'active'},{data:'action',name:'action',orderable:false,searchable:false,className:'text-end'}]});
