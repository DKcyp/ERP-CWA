    ...
    <div class="card border-0 shadow-sm hz-card mb-4"><div class="card-body">
        <div class="row g-3 align-items-end">
            <div class="col-md-4">
                ...
                <input type="text" class="form-control" id="filter-search" placeholder="Cari title atau module...">
            </div>
            <div class="col-md-8 d-flex gap-2 justify-content-md-end">
                ...
                <button type="button" class="btn btn-primary" id="btn-add"><i class="bi bi-plus-lg me-1"></i>Tambah Notes</button>
            </div>
        </div>
    </div></div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle w-100 table-sm" id="table-data">
                <thead class="table-light">
                    <tr><th class="text-center">No</th><th>Note ID</th><th>Title</th><th>Module</th><th>Default Text</th><th>Active</th><th class="text-end">Aksi</th></tr>
                </thead>
            </table>
        </div>
    </div></div>
...
<div class="modal fade" id="modal-data" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-lg modal-dialog-scrollable"><div class="modal-content">
        <div class="modal-header"><h5 class="modal-title">Tambah Notes</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <form id="form-data" action="javascript:onSave()">
            <div class="modal-body">@csrf<input type="hidden" id="data_id">
                <div class="row g-3">
                    <div class="col-6"><label class="form-label fw-semibold">Title <span class="text-danger">*</span></label><input type="text" class="form-control" name="title" id="f_title" maxlength="200"></div>
                    <div class="col-6"><label class="form-label fw-semibold">Module <span class="text-danger">*</span></label>
                        <select class="form-select" name="module" id="f_module">
                            <option value="">-- Pilih Module --</option>
                            <option value="Sales Order">Sales Order</option><option value="Purchase Order">Purchase Order</option>
                            <option value="Invoice">Invoice</option><option value="Delivery Order">Delivery Order</option>
                            <option value="Goods Receipt">Goods Receipt</option><option value="Stock Adjustment">Stock Adjustment</option>
                            <option value="Sales Return">Sales Return</option><option value="Purchase Return">Purchase Return</option>
                        </select>
                    </div>
                    <div class="col-12"><label class="form-label fw-semibold">Default Text</label><textarea class="form-control" name="default_text" id="f_default_text" rows="4" maxlength="1000"></textarea></div>
...
@push('after-script')
<script>
const tableUrl="{{route('notes.table')}}",storeUrl="{{route('notes.store')}}",showUrl="{{route('notes.show','__ID__')}}",updateUrl="{{route('notes.update','__ID__')}}",deleteUrl="{{route('notes.destroy','__ID__')}}",csrf=$('meta[name="csrf-token"]').attr('content');
...
{data:'note_id',name:'note_id'},{data:'title',name:'title'},{data:'module',name:'module'},
{data:'default_text',name:'default_text',render:function(d){return d?'<span class="text-truncate d-inline-block" style="max-width:300px">'+d+'</span>':'-'}},
...
function resetForm(){form[0].reset();idI.val('');form.find('.is-invalid').removeClass('is-invalid');form.find('.invalid-feedback').remove();modal.find('.modal-title').text('Tambah Notes');}
...
$('#table-data').on('click','.btn-edit',function(){const id=$(this).data('id');resetForm();$.get(showUrl.replace('__ID__',id)).done(function(r){const d=r.data||{};idI.val(d.id);$('#f_title').val(d.title??'');$('#f_module').val(d.module??'');$('#f_default_text').val(d.default_text??'');$('#f_active').val(d.active??'Y');modal.find('.modal-title').text('Edit Notes');modal.modal('show')}).fail(function(){Swal.fire({icon:'error',title:'Gagal',text:'Tidak dapat mengambil data.'})})});
