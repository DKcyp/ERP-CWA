...
@section('title','Product Cash Back')
@section('content')
    ...
    <div class="card border-0 shadow-sm hz-card mb-4"><div class="card-body">
        <div class="row g-3 align-items-end">
            <div class="col-md-4">
                ...
                <input type="text" class="form-control" id="filter-search" placeholder="Cari nama program, supplier, atau product...">
            </div>
            <div class="col-md-8 d-flex gap-2 justify-content-md-end">
                ...
                <button type="button" class="btn btn-primary" id="btn-add"><i class="bi bi-plus-lg me-1"></i>Tambah Cash Back</button>
            </div>
        </div>
    </div></div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle w-100 table-sm" id="table-data">
                <thead class="table-light">
                    <tr><th class="text-center">No</th><th>Cashback ID</th><th>Name</th><th>Supplier</th><th>Product</th><th>Min Purchase</th><th>Cashback Value</th><th>Valid From</th><th>Valid To</th><th>Active</th><th class="text-end">Aksi</th></tr>
                </thead>
            </table>
        </div>
    </div></div>
...
<div class="modal fade" id="modal-data" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-dialog-scrollable"><div class="modal-content">
        <div class="modal-header"><h5 class="modal-title">Tambah Cash Back</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <form id="form-data" action="javascript:onSave()">
            <div class="modal-body">@csrf<input type="hidden" id="data_id">
                <div class="row g-3">
                    ...
                    <div class="col-6"><label class="form-label fw-semibold">Min Purchase</label><input type="number" class="form-control" name="min_purchase" id="f_min_purchase" step="0.01" min="0"></div>
                    <div class="col-6"><label class="form-label fw-semibold">Cashback Value <span class="text-danger">*</span></label><input type="number" class="form-control" name="cashback_value" id="f_cashback_value" step="0.01" min="0"></div>
...
@push('after-script')
<script>
const tableUrl="{{route('product-cash-back.table')}}",storeUrl="{{route('product-cash-back.store')}}",showUrl="{{route('product-cash-back.show','__ID__')}}",updateUrl="{{route('product-cash-back.update','__ID__')}}",deleteUrl="{{route('product-cash-back.destroy','__ID__')}}",csrf=$('meta[name="csrf-token"]').attr('content');
...
{data:'cashback_id',name:'cashback_id'},{data:'name',name:'name'},{data:'supplier',name:'supplier'},{data:'product',name:'product'},
{data:'min_purchase',name:'min_purchase',render:function(d){return d?parseFloat(d).toLocaleString('id-ID'):'-'}},
{data:'cashback_value',name:'cashback_value',render:function(d){return d?'Rp '+parseFloat(d).toLocaleString('id-ID',{minimumFractionDigits:0}):'-'}},
...
function resetForm(){form[0].reset();idI.val('');form.find('.is-invalid').removeClass('is-invalid');form.find('.invalid-feedback').remove();modal.find('.modal-title').text('Tambah Cash Back');}
...
$('#table-data').on('click','.btn-edit',function(){const id=$(this).data('id');resetForm();$.get(showUrl.replace('__ID__',id)).done(function(r){const d=r.data||{};idI.val(d.id);$('#f_name').val(d.name??'');$('#f_supplier').val(d.supplier??'');$('#f_product').val(d.product??'');$('#f_min_purchase').val(d.min_purchase??'');$('#f_cashback_value').val(d.cashback_value??'');$('#f_valid_from').val(d.valid_from??'');$('#f_valid_to').val(d.valid_to??'');$('#f_active').val(d.active??'Y');modal.find('.modal-title').text('Edit Cash Back');modal.modal('show')}).fail(function(){Swal.fire({icon:'error',title:'Gagal',text:'Tidak dapat mengambil data.'})})});
$('#table-data').on('click','.btn-delete',function(){const id=$(this).data('id');Swal.fire({title:'Hapus cash back ini?',text:'Data yang dihapus tidak dapat dikembalikan.',icon:'warning',showCancelButton:true,confirmButtonColor:'#d33',confirmButtonText:'Ya, hapus',cancelButtonText:'Batal'}).then(function(r){if(!r.isConfirmed)return;$.ajax({url:deleteUrl.replace('__ID__',id),method:'POST',data:{_method:'DELETE'},success:function(r2){Swal.fire({icon:'success',title:r2.message||'Data dihapus',timer:1500,showConfirmButton:false});tbl.ajax.reload(null,false)},error:function(x){const r3=x.responseJSON||{};Swal.fire({icon:'error',title:'Gagal',text:r3.message||'Tidak dapat menghapus data.'})}})})});
</script>
@endpush