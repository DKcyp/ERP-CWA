...
class NotesController extends Controller
{
    ...
    {
        $this->store = new DummyStore('notes');
        View::share('activeMenu', 'notes');
    }

    public function index()
    {
        return view('master.notes.index');
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'default_text' => ['nullable','string','max:1000'],
            'active'       => ['required','string','in:Y,N'],
        ]);
        $this->store->create($request->only('title','module','default_text','active'));
    ...
    public function update(Request $request, $id)
    {
        ...
        $this->store->update($id, $request->only('title','module','default_text','active'));
