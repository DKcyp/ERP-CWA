...
class MonitoringSppbjController extends Controller
{
    protected DummyStore $store;

    public function __construct()
    {
        $this->store = new DummyStore('monitoring-sppbj');
        $this->initDummyData();
        View::share('activeMenu', 'monitoring-sppbj');
    }
    ...
    protected function initDummyData(): void
    {
        ...
        $types = ['CM','CM Pasta','CM Base'];
        $decisions = ['QC Approved','Reject CM','Rework ADU CM'];
        ...
        for ($d = 0; $d < 30; $d++) {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                $this->store->create([
                    ...
                    'batch_no' => 'CM-' . date('ymd', strtotime($date)) . '-' . strtoupper(substr(uniqid(), -4)),
                    ...
                    'colour' => ['#FFFFFF','#F5F5DC','#808080','#FFD700','#FF6347','#4682B4'][rand(0, 5)],
                    ...
                    'kesimpulan' => $dec === 'QC Approved' ? 'Produk CM memenuhi standar kualitas' : ($dec === 'Reject CM' ? 'Produk CM tidak lolos QC' : 'Perlu penyesuaian warna/kemasan'),
                    'keputusan' => $dec,
                    'note' => $dec === 'Reject CM' ? 'Matching test gagal, warna tidak sesuai standar' : ($dec === 'Rework ADU CM' ? 'Perlu penyesuaian viskositas warna' : '-'),
                ]);
            }
        }
    }
        ...
        return DataTables::of($data)
            ...
            ->addColumn('keputusan_badge', function ($r) {
                return match($r['keputusan'] ?? '') {
                    'QC Approved' => '<span class="badge bg-success"><i class="bi bi-check-circle me-1"></i>QC Approved</span>',
                    'Reject CM' => '<span class="badge bg-danger"><i class="bi bi-x-circle me-1"></i>Reject CM</span>',
                    'Rework ADU CM' => '<span class="badge bg-warning text-dark"><i class="bi bi-arrow-repeat me-1"></i>Rework ADU CM</span>',
    ...
    public function store(Request $request)
    {
        ...
        return response()->json(['success' => true, 'message' => 'Data monitoring SPPBJ berhasil disimpan.']);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return response()->json(['success' => true, 'message' => 'Data monitoring SPPBJ berhasil diperbarui.']);
    }
    ...
    public function destroy($id)
    {
        ...
        return response()->json(['success' => true, 'message' => 'Data monitoring SPPBJ berhasil dihapus.']);
    }
}
